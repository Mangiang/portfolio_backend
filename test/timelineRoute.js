        ...
        mongoClient.connect(mongoUrl, function(err, client){
            ...
            bcrypt.hash("test", 10, (err, hash) => {
                dbo.collection("user").insertOne({
                    ...
                    httpServer = http.createServer(app).listen(8888, () => { console.log("[timelineRoute][before] The server started on port 8888");
                    server.post("/user/login").send({ login:"test", password:"test" }).end((err, res) => { token = res.body.token; done(); });
                    });
                });
            })
